...
const Model3DS = forwardRef(
  ({ modelPath, onPartsLoaded, partSettings, drawMode, enable }, ref) => {
    ...
    // Load 3DS model
    const rawModel = useLoader(TDSLoader, modelPath);
    ...
    useEffect(() => {
      ...
      // Auto-fit model to a reasonable size and center it
      const box = new THREE.Box3().setFromObject(model);
      const size = box.getSize(new THREE.Vector3());
      const maxDim = Math.max(size.x, size.y, size.z) || 1;
      const scale = 2 / maxDim; // aim to fit within ~2 units
